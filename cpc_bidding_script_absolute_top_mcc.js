...
var BID_ADJUSTMENT_COEFFICIENT_UP = 1.10;

// How much to adjust the bids down.
var BID_ADJUSTMENT_COEFFICIENT_DOWN = 1.10;
...
function main() {
	var mccAccount =  AdWordsApp.currentAccount();
      var accountID = '***-***-****'; // Не забыть поменять!
...
function adjustKeywords() {
   var report = AdWordsApp.report(
    "SELECT CampaignName, CampaignId, AdGroupId, Id, Criteria, CpcBid, FirstPageCpc, SearchImpressionShare, SearchTopImpressionShare, SearchAbsoluteTopImpressionShare " +
...
function newCpcValue (Criteria, CpcBid, FirstPageCpc, ImpShare, TOPImpShare, AbsoluteTOPImpShare) {
  ...
  if (AbsoluteTOPImpShare >= TARGET_IMPRESSION_SHARE ) {
    cpc = CpcBid / BID_ADJUSTMENT_COEFFICIENT_DOWN;
  }
  
  if (AbsoluteTOPImpShare < (TARGET_IMPRESSION_SHARE - TOLERANCE)) {
